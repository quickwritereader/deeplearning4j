...
namespace sd {
    namespace sparse {
        ...
        template <typename T>
        sd::LongType SparseUtils<T>::coo_quickSort_findPivot(sd::LongType *indices, T *array, sd::LongType left, sd::LongType right,
                                                         int rank) {
        ...
        template<typename T>
        void SparseUtils<T>::coo_quickSort_parallel_internal(sd::LongType *indices, T* array, sd::LongType left, sd::LongType right, int cutoff, int rank) {
...
PRAGMA_OMP_TASK
        ...
        template <typename T>
        void SparseUtils<T>::coo_quickSort_parallel(sd::LongType *indices, T* array, sd::LongType lenArray, int numThreads, int rank){
...
PRAGMA_OMP_SINGLE_ARGS(nowait)
        ...
        BUILD_SINGLE_TEMPLATE(template class SD_LIB_HIDDEN SparseUtils, , SD_COMMON_TYPES);

        void IndexUtils::ravelMultiIndex(sd::LongType *indices, sd::LongType *flatIndices, sd::LongType length,  sd::LongType *shapeInfo, int mode){
        ...
        void IndexUtils::unravelIndex(sd::LongType *indices, sd::LongType *flatIndices, sd::LongType length,  sd::LongType *shapeInfo){
            sd::LongType * shape = shape::shapeOf(shapeInfo);
            sd::LongType * stride = shape::stride(shapeInfo);
            sd::LongType rank = shape::rank(shapeInfo);
            int errorCount = 0;
            ...
            if (errorCount > 0){
                // throw error if one ocurred in loop
                sd_printf("Largest raveled index is: %d, ", maxRaveledIndex)
                std::vector<sd::LongType> v(shape, shape + rank);
                sd_printv("Shape: ", v);
