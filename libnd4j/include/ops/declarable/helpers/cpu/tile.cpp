...
static void tileBP_(const NDArray& gradO /*input*/, NDArray& gradI /*output*/, const std::vector<sd::LongType> reps) {
    ...
    if(gradIEWS == 1)
        ...
        for (sd::LongType i = 0; i < gradILen * gradIEWS; i += gradIEWS)
...
BUILD_SINGLE_TEMPLATE(template void tileBP_, (const NDArray& gradO /*input*/, NDArray& gradI /*output*/, const std::vector<sd::LongType> reps), SD_FLOAT_TYPES);
