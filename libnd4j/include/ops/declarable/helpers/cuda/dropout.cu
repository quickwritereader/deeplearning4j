...
namespace helpers {
    ...
    template <typename T>
    static SD_KERNEL void dropoutSimpleKernel(void const* inputBuf, sd::LongType const* inputShape, void* outputBuf, sd::LongType const* outputShape, double probVal, int inLen, sd::graph::RandomGenerator* nodeRng) {
    ...
    template <typename T>
    sd::Status _dropOutFunctor(graph::Context& context, NDArray* input, NDArray* output, NDArray* reduceShape, int seed, double probValue) {
    ...
    sd::Status dropOutFunctor(graph::Context& context, NDArray* input, NDArray* output, NDArray* reduceShape, int seed, double probValue) {
        ...
        BUILD_SINGLE_SELECTOR(xType, return _dropOutFunctor, (context, input, output, reduceShape, seed, probValue), SD_FLOAT_TYPES);
...
/////////////////////////////////// backrpopagations ///////////////////////////////////////////////
    template <typename T>
    static SD_KERNEL void dropoutBPKernel(void* outputBuf, sd::LongType const* outputShape, void* gradOutBuf, sd::LongType const* gradOutShape, double probValue) {
        __shared__ T* output;
        __shared__ T* input;
        __shared__ int len;
    ...
    template <typename T>
    static SD_KERNEL void alphaDropoutSimpleKernel(void const* inputBuf, sd::LongType const* inputShape, void* outputBuf, sd::LongType const* outputShape, double probValue, double alpha, double alpha1, double beta, int inLen, sd::graph::RandomGenerator* nodeRng) {
        auto tid = blockIdx.x * blockDim.x + threadIdx.x;
        auto step = blockDim.x * gridDim.x;
        T const* input = reinterpret_cast<T const*>(inputBuf);
        T* output = reinterpret_cast<T*>(outputBuf);
