...
namespace sd
{
    namespace ops
    {
        namespace helpers
        {
            ...
            Type forward(Type *alphaPtr, const sd::LongType &incA, const Type *logP, const sd::LongType &incP, const IndexType *lbl, const sd::LongType &lenSB, const sd::LongType &lenT, const int &blankIndex, int elwiseP = 1, int elwiseS = 1)
            {
...
//#undef CALCULATE_ALL_IN_ONE_FRAME_LOOP
            ...
            void backwardAndGrad(Type forwardLogLoss, Type *alphaPtr, Type *bettaPtr, int incA,const Type *logP, int incP, Type *gradPtr, int incG,const IndexType *lbl,
                                 const sd::LongType &lenS, const sd::LongType &lenT, const sd::LongType &lenK, const int &blankIndex,
...
*/
           ...
           void ctcLoss(graph::Context& block, const NDArray &logits, const NDArray &targetLabels, const NDArray &logitsLengths, const NDArray &targetLabelLengths, NDArray &logLosses, NDArray &gradients, int blankIndex){
                ...
                BUILD_DOUBLE_SELECTOR(logits.dataType(), targetLabels.dataType(), ctc_loss_, (logits, targetLabels, logitsLengths, targetLabelLengths, logLosses, gradients, blankIndex), SD_FLOAT_TYPES, SD_INDEXING_TYPES);
            }


            BUILD_DOUBLE_TEMPLATE(template void ctc_loss_, (const NDArray &logits, const NDArray &targetLabels, const NDArray &logitsLengths, const NDArray &targetLabelLengths, NDArray &logLosses, NDArray &gradients, int blankIndex), SD_FLOAT_TYPES, SD_INDEXING_TYPES);
