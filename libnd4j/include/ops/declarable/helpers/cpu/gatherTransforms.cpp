...
void gatherND(sd::LaunchContext * context, NDArray& input, NDArray& indices, NDArray& output) {
    BUILD_DOUBLE_SELECTOR(input.dataType(), indices.dataType(), gatherND_, (input, indices, output), SD_COMMON_TYPES, SD_INDEXING_TYPES);
}
    ...
    if (indices != nullptr) {
        ...
        if(indices->isScalar()) {
            if(input->rankOf() <= 1){
                ...
                auto tadArr = NDArray(reinterpret_cast<void *>(reinterpret_cast<T*>(input->buffer()) + tadPack.primaryOffsets()[indices->e<sd::LongType>(0)]), tadPack.primaryShapeInfo(), output->getContext());
            ...
            auto func = PRAGMA_THREADS_FOR {
                for (auto e = start; e < stop; e++)
                    output->p(e, input->e<T>(indices->e<sd::LongType>(e)));
            };
