...
namespace helpers {
    ...
    template <typename T>
    static SD_DEVICE void adjustWeightsKernelD(void* inputBuffer,   sd::LongType const*  inputShape,
                                               void* weightsBuffer, sd::LongType const*  weightsShape,
                                               void* outputBuffer,  sd::LongType inputLength,
                                               sd::LongType outputLength, int val) {
        ...
        for (sd::LongType e = tid; e < inputLength; e += blockDim.x) {
            ...
            if (current == val) {
                ...
                if (weightsBuffer != nullptr) {
                    ...
                    sd::math::atomics::sd_atomicAdd(reinterpret_cast<T *>(outputBuffer), reinterpret_cast<T *>(weightsBuffer)[yOffset]); //output->p(val, output->e<T>(val) + 1);
//                    atomicAdd(reinterpret_cast<int *>(outputBuffer), reinterpret_cast<int *>(weightsBuffer)[yOffset]); //output->p(val, output->e<T>(val) + 1);
                    ...
                    sd::math::atomics::sd_atomicAdd(reinterpret_cast<T *>(outputBuffer), T(1)); //output->p(val, output->e<T>(val) + 1);
...
//            reinterpret_cast<T *>(outputBuffer)[threadIdx.x + offset] = outputPart[threadIdx.x];
        ...
        template <typename T>
    static SD_KERNEL void adjustWeightsKernel(void* inputBuffer,   sd::LongType const*  inputShape,
                                               void* weightsBuffer, sd::LongType const*  weightsShape,
                                               void* outputBuffer,  sd::LongType const*  outputShape,
...
//        }
    ...
    void adjustWeights(sd::LaunchContext * context, NDArray* input, NDArray* weights, NDArray* output, int minLength, int maxLength) {
        BUILD_SINGLE_SELECTOR(output->dataType(), adjustWeights_, (context, input, weights, output, minLength, maxLength), SD_GENERIC_NUMERIC_TYPES);
    }

    BUILD_SINGLE_TEMPLATE(template void adjustWeights_, (sd::LaunchContext * context, NDArray* input, NDArray* weights, NDArray* output, int minLength, int maxLength), SD_GENERIC_NUMERIC_TYPES);
}
}
}