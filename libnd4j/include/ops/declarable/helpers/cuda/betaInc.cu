...
SD_DEVICE T continuedFractionCuda(const T a, const T b, const T x) {
    ...
    for(sd::Unsigned i = 1; i <= maxIter; ++i) {
        ...
        if(math::sd_abs<T>(val - static_cast<T>(1)) <= DataTypeUtils::eps<T>())
            return result;
    }
...
///////////////////////////////////////////////////////////////////
template<typename T>
SD_KERNEL void betaIncForArrayCuda(const void* va, const sd::LongType* aShapeInfo,
...
static void betaIncForArrayCudaLauncher(const int blocksPerGrid, const int threadsPerBlock, const int sharedMem, const cudaStream_t *stream,
                                        const void* va, const sd::LongType* aShapeInfo,
                                        const void* vb, const sd::LongType* bShapeInfo,
                                        const void* vx, const sd::LongType* xShapeInfo,
                                                void* vz, const sd::LongType* zShapeInfo) {
...
void betaInc(sd::LaunchContext* context, const NDArray& a, const NDArray& b, const NDArray& x, NDArray& output) {
    ...
    BUILD_SINGLE_SELECTOR(xType, betaIncForArrayCudaLauncher, (blocksPerGrid, threadsPerBlock, sharedMem, context->getCudaStream(), a.specialBuffer(), a.specialShapeInfo(), b.specialBuffer(), b.specialShapeInfo(), x.specialBuffer(), x.specialShapeInfo(), output.specialBuffer(), output.specialShapeInfo()), SD_FLOAT_TYPES);
