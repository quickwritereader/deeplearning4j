...
template<typename X>
SD_KERNEL static void checkIndicesCuda(const void *vx, const sd::LongType *xShapeInfo, sd::LongType* y, const sd::LongType *zShapeInfo, const int axis) {
...
static void checkIndicesCudaLauncher(const int blocksPerGrid, const int threadsPerBlock, const int sharedMem, const cudaStream_t *stream,
                                              const void *vx, const sd::LongType *xShapeInfo, sd::LongType* y, const sd::LongType *zShapeInfo, const int axis) {
...
sd::LongType checkIndices(sd::LaunchContext *context, const NDArray& indices, const NDArray& output, const int axis) {
    ...
    BUILD_SINGLE_SELECTOR(xType, checkIndicesCudaLauncher, (blocksPerGrid, threadsPerBlock, sharedMem, context->getCudaStream(), indices.specialBuffer(), indices.specialShapeInfo(), reinterpret_cast<sd::LongType*>(numOfBadIndx.specialBuffer()), output.specialShapeInfo(), axis), SD_INDEXING_TYPES);
    ...
    for (sd::LongType i = blockIdx.x * blockDim.x + threadIdx.x; i < zLen; i += gridDim.x * blockDim.x) {
        ...
        for (sd::LongType j = 0; j < xLen; ++j) {
            ...
            if(is1Dcase) {
                ...
                for (sd::Unsigned k = 0; k < yRank - xRank; ++k)
...
// x - indices, y - updates, z - input/output
template<typename X, typename Y>
SD_KERNEL static void scatterCuda(const int opCode,
                                   const void *vx, const sd::LongType *xShapeInfo,
                                   const void *vy, const sd::LongType *yShapeInfo,
                                         void *vz, const sd::LongType *zShapeInfo) {
    ...
    for (sd::LongType i = blockIdx.x * blockDim.x + threadIdx.x; i < yLen; i += gridDim.x * blockDim.x) {
        ...
        if(is1Dcase) {
            ...
            yOffset = shape::getOffset(yShapeInfo, yCoords);
            xOffset = shape::getOffset(xShapeInfo, yCoords);                // first xRank coordinates in yCoords are the same for y and x -> for (sd::Unsigned j = 0; j < xRank; ++j) xCoords[j] = yCoords[j];

            zCoords[0] = x[xOffset];

            for (sd::Unsigned j = 0; j < yRank - xRank; ++j)
...
SD_KERNEL static void scatterNDLockCuda(const int opCode,
    ...
    for (sd::LongType i = blockIdx.x * blockDim.x + threadIdx.x; i < zLen; i += gridDim.x * blockDim.x) {
        ...
        for (sd::LongType j = 0; j < len; ++j) {        // if !is1Dcase then we loop through first xRank-1 dimensions of x, that is we exclude last x dimension
            ...
            if(is1Dcase) {
                ...
                shape::index2coords(j, xRank-1, shape::shapeOf(const_cast<sd::LongType*>(xShapeInfo)), yCoords);        // first xRank-1 coordinates in yCoords are the same for y and x
                ...
                for (sd::Unsigned k = xLastDim; k < zRank; ++k)
...
// x - indices, y - updates, z - output
template<typename X, typename Y>
SD_KERNEL static void scatterNDCuda(const int opCode,
                                     const void *vx, const sd::LongType *xShapeInfo,
                                     const void *vy, const sd::LongType *yShapeInfo,
                                           void *vz, const sd::LongType *zShapeInfo) {
    ...
    for (sd::LongType i = blockIdx.x * blockDim.x + threadIdx.x; i < yLen; i += gridDim.x * blockDim.x) {
        ...
        if(is1Dcase) {
            ...
            for (sd::Unsigned j = xLastDim + 1; j < zRank; ++j)
...
SD_KERNEL void scatterForLossCuda(const void *vx, const sd::LongType *xShapeInfo,
                                         void *vy, const sd::LongType *yShapeInfo,
                                         void *vz, const sd::LongType *zShapeInfo) {
...
///////////////////////////////////////////////////////////////////
template<typename X, typename Z>
static void scatterForLossCudaLauncher(const int blocksPerGrid, const int threadsPerBlock, const int sharedMem, const cudaStream_t *stream, const void *vx, const sd::LongType* xShapeInfo, void *vy, const sd::LongType* yShapeInfo, void *vz, const sd::LongType* zShapeInfo) {
...
void scatterForLoss(sd::LaunchContext* context, const NDArray& indices, NDArray& updates, NDArray& output, const bool calcGrad) {
    ...
    if(calcGrad) {
        ...
        BUILD_DOUBLE_SELECTOR(indices.dataType(), updates.dataType(), scatterForLossCudaLauncher, (blocksPerGrid, threadsPerBlock, sharedMem, context->getCudaStream(), indices.specialBuffer(), indices.specialShapeInfo(), updates.specialBuffer(), updates.specialShapeInfo(), nullptr, nullptr), SD_INDEXING_TYPES, SD_FLOAT_TYPES);
        NDArray::registerSpecialUse({&updates}, {&indices});
    }
    else {
        NDArray::prepareSpecialUse({&output}, {&indices, &updates});
        BUILD_DOUBLE_SELECTOR(indices.dataType(), updates.dataType(), scatterForLossCudaLauncher, (blocksPerGrid, threadsPerBlock, sharedMem, context->getCudaStream(), indices.specialBuffer(), indices.specialShapeInfo(), updates.specialBuffer(), updates.specialShapeInfo(), output.specialBuffer(), output.specialShapeInfo()), SD_INDEXING_TYPES, SD_FLOAT_TYPES);
...
SD_KERNEL static void scatterLockCuda(const int opCode,
 ...
 const int xRank = indices.rankOf();
        ...
        BUILD_DOUBLE_SELECTOR(xType, yType, scatterLockCudaLauncher, (blocksPerGrid, threadsPerBlock, 1024, context->getCudaStream(), op, indices.specialBuffer(), indices.specialShapeInfo(), updates.specialBuffer(), packY.specialShapeInfo(), packY.specialOffsets(), output.specialBuffer(), packZ.specialShapeInfo(), packZ.specialOffsets(), indices.lengthOf(), yTadLen, zTadLen), SD_INDEXING_TYPES, SD_GENERIC_NUMERIC_TYPES);
...
SD_KERNEL static void scatterNDLockCuda(const int opCode,
                                         ...
                                         const sd::LongType numOfXTads, const sd::LongType numOfZTads, const sd::LongType yTadLen) {
