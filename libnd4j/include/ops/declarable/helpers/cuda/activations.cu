...
void SD_KERNEL preluCuda(const void *vx, const sd::LongType *xShapeInfo,
...
///////////////////////////////////////////////////////////////////
template<typename X, typename Y>
 void preluCudaLauncher(const int blocksPerGrid, const int threadsPerBlock, const int sharedMem, const cudaStream_t *stream, const void *vx, const sd::LongType *xShapeInfo, const void *vy, const sd::LongType *yShapeInfo, void *vz) {
...
void prelu(sd::LaunchContext * context, const NDArray& input, const NDArray& alpha, NDArray& output) {
    ...
    BUILD_SINGLE_SELECTOR_TWICE(xType, preluCudaLauncher, (blocksPerGrid, threadsPerBlock, sharedMem, context->getCudaStream(), input.specialBuffer(), input.specialShapeInfo(), alpha.specialBuffer(), alpha.specialShapeInfo(), output.specialBuffer()), SD_FLOAT_TYPES);
...
///////////////////////////////////////////////////////////////////
template<typename X, typename Y>
void SD_KERNEL preluBPCuda(const void *vIn,    const sd::LongType *inShapeInfo,
                                   const void *vAlpha, const sd::LongType *alphaShapeInfo,
                                   const void *vdLdO,  const sd::LongType *dLdOShapeInfo,
                                         void *vdLdI,  const sd::LongType *dLdIShapeInfo,
...
//////////////////////////////////////////////////////////////////////////
template<typename X, typename Y>
void SD_HOST preluBPCudaLauncher(const int blocksPerGrid, const int threadsPerBlock, const int sharedMem, const cudaStream_t *stream, const void *vIn, const sd::LongType *inShapeInfo, const void *vAlpha, const sd::LongType *alphaShapeInfo, const void *vdLdO,  const sd::LongType *dLdOShapeInfo, void *vdLdI,  const sd::LongType *dLdIShapeInfo, void *vdLdA,  const sd::LongType *dLdAShapeInfo) {
...
///////////////////////////////////////////////////////////////////
template<typename T>
SD_DEVICE void softMaxForVectorCuda(const void *vx, const sd::LongType *xShapeInfo, void *vz, const sd::LongType *zShapeInfo) {
    ...
    for (int i = 0; i < numOfIters; ++i) {
        ...
        if(elemIdx < len) {
            ...
            shmem[threadIdx.x] = (threadIdx.x != 0) ? x[xOffset] : sd::math::sd_max<T>(x[xOffset], temp);    // take into account max element evaluated on previous iteration and stored in temp
        }
        else
        ...
        for (int s = blockDim.x / 2; s > 0; s /= 2) {
            if(threadIdx.x < s)
                shmem[threadIdx.x] = sd::math::sd_max<T>(shmem[threadIdx.x], shmem[threadIdx.x + s]);
            __syncthreads();
        }
...
template<typename T>
void SD_KERNEL softMaxForVectorCudaGlobal(const void *vx, const sd::LongType *xShapeInfo, void *vz, const sd::LongType *zShapeInfo) {
...
///////////////////////////////////////////////////////////////////
template <typename T>
 void softMaxForVectorCudaLauncher(const cudaStream_t* stream, const void *vx, const sd::LongType *xShapeInfo, void *vz, const sd::LongType *zShapeInfo) {
...
static void softMaxCudaLauncher(const int blocksPerGrid, const int threadsPerBlock, const int sharedMem, const cudaStream_t *stream,
                                const void* vx, const sd::LongType *xTadShapeInfo, const sd::LongType *xOffsets,
                                      void* vz, const sd::LongType *zTadShapeInfo, const sd::LongType *zOffsets) {
...
void softmax(sd::LaunchContext * context, const NDArray& input, NDArray& output, const int dimension) {
    ...
    if(input.isVector()) {
        ...
        if(rank == 1 || input.sizeAt(dimension) != 1) {
            ...
            BUILD_SINGLE_SELECTOR(input.dataType(), softMaxForVectorCudaLauncher, (context->getCudaStream(), input.specialBuffer(), input.specialShapeInfo(), output.specialBuffer(), output.specialShapeInfo()), SD_FLOAT_TYPES);
        ...
        NDArray::prepareSpecialUse({&output}, {&input});
        BUILD_SINGLE_SELECTOR(input.dataType(), softMaxCudaLauncher, (blocksPerGrid, threadsPerBlock, sharedMem, context->getCudaStream(), input.specialBuffer(), packX.specialShapeInfo(), packX.specialOffsets(), output.specialBuffer(), packZ.specialShapeInfo(), packZ.specialOffsets()), SD_FLOAT_TYPES);
        NDArray::registerSpecialUse({&output}, {&input});
...
void SD_KERNEL logSoftMaxForVectorCuda(const void *vx, const sd::LongType *xzShapeInfo, void *vz) {
    ...
    for (int i = 0; i < numOfIters; ++i) {
        ...
        if(elemIdx < len) {
            ...
            shmem[threadIdx.x] = (threadIdx.x != 0) ? x[offset] : sd::math::sd_max<T>(x[offset], temp);    // take into account max element evaluated on previous iteration and stored in temp
        }
        else
            shmem[threadIdx.x] = -DataTypeUtils::max<T>();    // FIXME: what if T is unsigned ??
...
///////////////////////////////////////////////////////////////////
template <typename T>
 void logSoftMaxForVectorCudaLauncher(const cudaStream_t* stream, const void *vx, const sd::LongType *xzShapeInfo, void *vz) {
...
void logSoftmax(sd::LaunchContext * context, const NDArray& input, NDArray& output, const int dimension) {
    ...
    if(input.isVector()) {
        ...
        if(rank == 1 || input.sizeAt(dimension) != 1) {
            BUILD_SINGLE_SELECTOR(input.dataType(), logSoftMaxForVectorCudaLauncher, (context->getCudaStream(), input.specialBuffer(), input.specialShapeInfo(), output.specialBuffer()), SD_FLOAT_TYPES);
            input.tickReadDevice();
        }
        else
            output = 0.;
...
///////////////////////////////////////////////////////////////////
template <typename T>
 void softMaxDerivForVectorCudaLauncher(const cudaStream_t* stream, const void *vx, const sd::LongType *xzShapeInfo, void *vz) {
...
void softmaxDerivative(sd::LaunchContext * context, const NDArray& input, NDArray& output, const int dimension) {
    ...
    if(shape::isCommonVector(input.shapeInfo(), temp)) {
        ...
        BUILD_SINGLE_SELECTOR(input.dataType(), softMaxDerivForVectorCudaLauncher, (context->getCudaStream(), input.specialBuffer(), input.specialShapeInfo(), output.specialBuffer()), SD_FLOAT_TYPES);
        input.tickReadDevice();
    }
    else {
