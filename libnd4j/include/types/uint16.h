...
//////////////////// IMPLEMENTATIONS
    ...
    float SD_HOST_DEVICE cpu_uint162float(uint16_t data) {
    ...
    SD_HOST_DEVICE uint16::operator float() const {
        return cpu_uint162float(data);
    }

    SD_HOST_DEVICE void uint16::assign(float rhs) {
        data = cpu_float2uint16(rhs);
    }

    SD_HOST_DEVICE void uint16::assign(double rhs) {
        assign((float)rhs);
    }
}
