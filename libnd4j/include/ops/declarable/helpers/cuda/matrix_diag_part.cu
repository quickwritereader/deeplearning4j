...
// put diagonals from input batched matricies to output batched vectors
    template <typename T>
    static SD_KERNEL void matrixDiagPartKernel(void const* inputBuffer, void* outputBuffer, sd::LongType numTads, sd::LongType inputLength,
...
//
    ...
    static sd::Status _matrixDiagPart(sd::LaunchContext * context, const NDArray* input, NDArray* output) {
        ...
        const sd::LongType numTads = ShapeUtils::getNumOfSubArrs(input->shapeInfo(), dimsToExclude); //this->tensorsAlongDimension({dimension});
...
//
    ...
    BUILD_SINGLE_TEMPLATE(template sd::Status _matrixDiagPart, (sd::LaunchContext * context, const NDArray* input, NDArray* output), SD_COMMON_TYPES);
