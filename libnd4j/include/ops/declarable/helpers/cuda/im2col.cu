...
template <typename T>
SD_KERNEL static void im2colCuda(const void *image, void *columns,
                                  const sd::LongType *imShapeInfo, const sd::LongType *colShapeInfo,
...
//////////////////////////////////////////////////////////////////////////
template <typename T>
static void im2colCudaLauncher(const int blocksPerGrid, const int threadsPerBlock, sd::LaunchContext & context, const void *image, void *columns, const sd::LongType *imShapeInfo, const sd::LongType *colShapeInfo, int sH, int sW, int pH, int pW, int dH, int dW, double zeroPadVal) {
...
void im2col(sd::LaunchContext& context, const NDArray& image, NDArray& columns, const int kH, const int kW, const int sH, const int sW, const int pH, const int pW, const int dH, const int dW, const NDArray& arrZeroPadVal) {
    ...
    BUILD_SINGLE_SELECTOR(columns.dataType(), im2colCudaLauncher, (blocksPerGrid, threadsPerBlock, context, image.specialBuffer(), columns.specialBuffer(), image.specialShapeInfo(), columns.specialShapeInfo(), sH, sW, pH, pW, dH, dW, arrZeroPadVal.e<double>(0)), SD_FLOAT_TYPES);
