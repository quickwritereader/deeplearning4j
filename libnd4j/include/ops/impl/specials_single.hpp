...
*/
template <typename T>
void SpecialMethods<T>::concatCpuGeneric(int dimension, int numArrays, sd::Pointer *data, sd::Pointer *inputShapeInfo, void *vresult, sd::LongType const* resultShapeInfo) {
 ...
 */
    template<typename T>
    void SpecialMethods<T>::accumulateGeneric(void **vx, void *vz, sd::LongType const* zShapeInfo, int n, const sd::LongType length) {
 ...
 */
    template<typename T>
    void SpecialMethods<T>::averageGeneric(void **vx, void *vz, sd::LongType const* zShapeInfo, int n, const sd::LongType length, bool propagate) {
        auto z = reinterpret_cast<T *>(vz);
        auto x = reinterpret_cast<T **>(vx);
    ...
    template<typename T>
    void SpecialMethods<T>::quickSort_parallel_internal(T* array, sd::LongType const* xShapeInfo, int left, int right, int cutoff, bool descending) {
...
PRAGMA_OMP_TASK
    ...
    template<typename T>
    void SpecialMethods<T>::quickSort_parallel(void *varray, sd::LongType const* xShapeInfo, sd::LongType lenArray, int numThreads, bool descending){
...
PRAGMA_OMP_SINGLE_ARGS(nowait)
    ...
    template<typename T>
    void SpecialMethods<T>::sortTadGeneric(void *vx, sd::LongType const* xShapeInfo, int *dimension, int dimensionLength, sd::LongType const* tadShapeInfo, sd::LongType const* tadOffsets, bool descending) {
        auto x = reinterpret_cast<T *>(vx);
    ...
    template<typename T>
    sd::LongType SpecialMethods<T>::encodeBitmapGeneric(void *vx, sd::LongType const* xShapeInfo, sd::LongType N, int *dz, float threshold) {
