...
namespace sd {
    namespace graph {
        ...
        public:
             ...
             */
            SD_INLINE SD_HOST void setSeed(int seed) {
                _nodeState._ulong = static_cast<uint64_t>(seed);
            }

            SD_INLINE SD_HOST void setSeed(uint64_t seed) {
                _nodeState._ulong = seed;
            }

            SD_INLINE SD_HOST_DEVICE sd::LongType rootState() {
                return _rootState._long;
            }

            SD_INLINE SD_HOST_DEVICE sd::LongType nodeState() {
        ...
        template <>
        SD_INLINE SD_HOST_DEVICE sd::LongType RandomGenerator::relativeT(sd::LongType index, sd::LongType from, sd::LongType to) {
        ...
        //////
        static SD_INLINE SD_HOST_DEVICE uint32_t rotl(const uint32_t x, int k) {
            return (x << k) | (x >> (32 - k));
        }
