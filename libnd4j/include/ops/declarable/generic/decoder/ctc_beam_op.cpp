...
CUSTOM_OP_IMPL(ctc_beam, 2, 3, false, 0, -2) {
    ...
    int batchSize0 = logit->sizeAt(0);
    int batchSize1 = sequence_length->sizeAt(0);
    int batchSize2 = result_sequences->sizeAt(0);
    int batchSize3 = result_probs->sizeAt(0);
    int batchSize4 = result_sequences_length->sizeAt(0);
...
DECLARE_SHAPE_FN(ctc_beam) {
    ...
    int batch_size = shape::shapeOf(logitShapeInfo)[0] ;
    int max_t = shape::shapeOf(logitShapeInfo)[1] ;
